//! End-to-end tests of `datom check`: spawns the real binary against source
...
/// Write `source` to `dir/input.datom` and run `datom check` on it.
fn check(dir: &Path, source: &str) -> Output {
    ...
    Command::new(env!("CARGO_BIN_EXE_datom"))
        .args(["check", file.to_str().expect("path should be utf-8")])
        .output()
...
fn a_parse_error_fails_and_names_what_it_wanted() {
    let dir = tempdir().unwrap();
    let output = check(dir.path(), "type Person(name string)\n");
